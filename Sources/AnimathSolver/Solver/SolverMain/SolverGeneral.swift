...
//  Hulul
//
//  Created by Ahmad on 27/02/2021.
//  Copyright © 2021 Ahmad. All rights reserved.
//

extension CalcBrain {
    var setExponentToOneExplanation: String {"If a term doesn't have an exponent, its exponent is 1"}
    var UseCommutativePropExplanation: String {"Use the commutative property to reorder the terms"}
    var rewriteInExponentialExplanation: String {"Rewrite the number in exponential form"}
    var radToExpExplanation: String {"Use ⁿ√aᵐ = aᵐᐟⁿ to transform the expression"}
    var multTermsWithSameBaseExpl: String {"To multiply terms with the same base, add their exponents"}
    var multToExpFormExpl: String {"Express the repeated multiplication in exponential form"}
    var calcProdExpl: String {"Calculate the product"} // Used in marked nodes for multSameBase(node:...)
    func setToUndefined(nodeL: StepNode, nodeR: StepNode, markedKeys: [StepKey], explanation: String, _ steps: inout [StepModel]) {
        steps.lastMarked = markedKeys
        steps.lastExplanation = explanation
        steps[steps.count-1].nodeL.isUndefined = true
        nodeL.isUndefined = true
        steps[steps.count-1].nodeR.isUndefined = true
        nodeR.isUndefined = true
        if steps.count == 2 && steps.first!.hasSubSteps {
            steps[0].subSteps.lastStep.markedKeys.append(contentsOf: markedKeys)
            steps[0].subSteps.lastStep.explanation = explanation
        }
    }
}

extension CalcBrain {
    func stepsInit(nodeL: StepNode, nodeR: StepNode, fnCtrl: [FnCtrl], steps: inout [StepModel]) {
        nodeL.otherSide = nodeR
        nodeR.otherSide = nodeL
        replaceXWithNotVarXIfApplicable(nodeL: nodeL)
        steps[0].nodeL = nodeL
        steps[0].nodeR = nodeR
        steps[0].dynamicNodeL = nodeL
        steps[0].dynamicNodeR = nodeR
        appendStep(&steps, fnCtrl: fnCtrl + [.setInMainSteps])
    }
}
